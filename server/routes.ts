...
import { insertFileSchema, insertConversationSchema, insertMessageSchema, insertNoteSchema, insertAssignmentConversationSchema, insertAssignmentMessageSchema, insertAssignmentFileSchema } from "@shared/schema";
...
If no valuable insights are found, return: {"items": []}`;
  ...
  // Assignment conversation routes
  
  // Create assignment conversation and get feedback
  app.post("/api/assignment-conversations", isAuthenticated, upload.array('files'), async (req: any, res) => {
    try {
      const userId = req.user?.id;
      const { sessionId, courseId, assignmentText, message } = req.body;
      const files = req.files as Express.Multer.File[];

      // Get or create conversation
      let conversation = await storage.getAssignmentConversationBySession(sessionId);
      
      if (!conversation) {
        conversation = await storage.createAssignmentConversation({
          sessionId,
      ...
      // Upload files to object storage and create file records
      const uploadedFiles = [];
      if (files && files.length > 0) {
        for (const file of files) {
          ...
          uploadedFiles.push(assignmentFile);
        }
      }

      // Create user message
      const userMessage = await storage.createAssignmentMessage({
        conversationId: conversation.id,
        role: 'user',
        content: message
      });

      // Generate AI feedback using assignment text as system prompt
      const genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY || "");
      const model = genAI.getGenerativeModel({ model: "gemini-2.0-flash-exp" });

      const systemPrompt = `Provide the user with critical and constructive feedback on the work they have submitted in response to this assignment brief: 

${assignmentText}

Focus on:
- How well the submission addresses the assignment requirements
- Technical execution and craftsmanship
- Creative and conceptual strengths
- Areas for improvement with specific suggestions
- Encouraging next steps for further development

Be thorough, supportive, and educational in your feedback.`;

      const userPrompt = message + (uploadedFiles.length > 0 ? ` (${uploadedFiles.length} file(s) uploaded)` : '');

      const result = await model.generateContent([
        { text: systemPrompt },
        { text: `User submission: ${userPrompt}` }
      ]);
      ...
      const aiMessage = await storage.createAssignmentMessage({
        conversationId: conversation.id,
        role: 'assistant',
        content: aiResponse
      });
      ...
      const messages = await storage.getAssignmentMessagesByConversation(conversation.id);
      const conversationFiles = await storage.getAssignmentFilesByConversation(conversation.id);
      ...
      console.error("Assignment conversation error:", error);
      res.status(500).json({ 
        message: "Failed to process assignment submission",
        error: error instanceof Error ? error.message : "Unknown error"
      });
    }
  });

  // Get assignment conversation by session ID
  app.get("/api/assignment-conversations/:sessionId", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      const { sessionId } = req.params;

      const conversation = await storage.getAssignmentConversationBySession(sessionId);
      ...
      const messages = await storage.getAssignmentMessagesByConversation(conversation.id);
      const files = await storage.getAssignmentFilesByConversation(conversation.id);
      ...
      console.error("Get assignment conversation error:", error);
      res.status(500).json({ message: "Failed to get conversation" });
    }
  });

  // Get user's assignment conversations
  app.get("/api/assignment-conversations", isAuthenticated, async (req: any, res) => {
    try {
      const userId = req.user?.id;
      const conversations = await storage.getAssignmentConversationsByUser(userId);
      
      res.json({ conversations });
    } catch (error) {
      console.error("Get assignment conversations error:", error);
      res.status(500).json({ message: "Failed to get conversations" });
    }
  });
